...
for scenario in scenarios:
     for source in sources:
          ...
          for variable in variables:
                ...
                file_name = path2 + "/" + scenario + "/" + source + "/0"  + variable + "_" + scenario + "_" + source
                with open(file_name + '.txt', 'w') as f:
                    txt = "ID,NAME,LAT,LONG,ELEVATION"
                    f.write(txt + "\n")
                    for j in range(4,20):  # j为纬度
                        for k in range(16,33):  # k为经度
                            mm = mm + 1
                            f_name = variable + "_" + scenario + "_" + source + "_lat" + str(j).zfill(3) + "_lon" + str(k).zfill(3)
                            lat = lat_set[j]
                            lon = lon_set[k]

                            txt = str(mm) + "," + f_name + ","+ str(round(lat,3)) + ","+ str(round(lon,3)) + ","+ "100.000"
                            f.write(txt + "\n")
